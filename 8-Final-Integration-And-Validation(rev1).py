# 8-Final-Integration-And-Validation(rev1).py
...
def build_step7_lookup(step7):
    ...
    key_cols = ["competitor_name", "competitor_name_standardized"]
    missing = [col for col in key_cols if col not in step7.columns]
    if missing:
        raise ValueError(f"Step 7 file is missing required merge columns: {missing}")

    step7["competitor_name_key"] = step7["competitor_name"].apply(normalize_key)
    step7["competitor_name_standardized_key"] = step7["competitor_name_standardized"].apply(normalize_key)

    sort_cols = [col for col in ["frequency", "llm_confidence"] if col in step7.columns]
    if sort_cols:
        step7 = step7.sort_values(sort_cols, ascending=[False] * len(sort_cols))

    keep_cols = (
        ["competitor_name_key", "competitor_name_standardized_key"]
        + STEP7_COLUMNS
        + STEP7_REVIEW_COLUMNS
    )
    step7 = step7[keep_cols].drop_duplicates(
        subset=["competitor_name_key", "competitor_name_standardized_key"],
        keep="first",
    )
    return step7
...
def build_alias_candidates(long):
    candidates = long[long["final_match_stage"] == "llm_alias_candidate_unresolved"].copy()
    requested_cols = [
        "competitor_name",
        "competitor_name_standardized",
        "suggested_alias_name",
    ...
    candidates = ensure_columns(candidates, requested_cols, "")
    if candidates.empty:
        out = pd.DataFrame(columns=requested_cols)
    else:
        group_cols = [
            "competitor_name",
            "competitor_name_standardized",
        ...
        out = (
            ...
            .agg(
                frequency=("frequency", lambda x: first_nonblank_or_count(x, len(x))),
                sic_files=("sic_files", collapse_values),
                years=("years", collapse_values),
                example_filename=("example_filename", lambda x: first_nonblank(x)),
            )
            .reset_index()
        )
        out = out[requested_cols]
...
def main():
    ...
    long["competitor_name_key"] = long["competitor_name"].apply(normalize_key)
    long["competitor_name_standardized_key"] = long["competitor_name_standardized"].apply(normalize_key)

    if not step7_lookup.empty:
        long = long.merge(
            step7_lookup,
            on=["competitor_name_key", "competitor_name_standardized_key"],
            how="left",
        )
    ...
    unresolved_mask = (~long["step5_has_any_identifier"]) & (~long["final_has_any_identifier"])
    ...
    final_dropped = long[long["final_match_stage"] == "llm_dropped_residual"].copy()
    final_manual_review = long[long["final_match_stage"] == "llm_manual_review_unresolved"].copy()
    ...
    sample_with_notes(
        long[long["final_match_stage"] == "llm_alias_candidate_unresolved"],
