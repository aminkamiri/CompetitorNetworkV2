# 8.5-Alias-Candidate-Generation(rev10).py
...
def add_approved_abbreviation_candidates(candidates, lookups, alias_name, years):
    ...
    for search_name in entry.get("search_names", []):
        ...
        for candidate in candidate_batch:
            source = str(candidate.get("candidate_source", "")).upper()
            if source not in STRONG_BRIDGE_SOURCES:
                continue
            identity = candidate_identity(candidate) + (candidate.get("candidate_match_rule"), search_name)
            if identity in seen:
                continue
            seen.add(identity)
            candidates.append(apply_abbreviation_audit(candidate, raw_alias, search_name, confidence, "strict_high_confidence_abbreviation_year"))
...
def find_candidates(alias_name, lookups, years, validation_panel):
    ...
    add_approved_abbreviation_candidates(abbreviation_candidates, lookups["year_specific"], alias_name, years)
    abbreviation_candidates = dedupe_candidates(abbreviation_candidates)
    if abbreviation_candidates:
        return annotate_identifier_year_panel_presence(abbreviation_candidates, years, validation_panel)
...
def assign_acceptance_tier(row):
    ...
    if family == "approved_abbreviation":
        ...
        if n_year == 1 and has_panel and strong_source and confidence == "high":
            return "Tier TIC-SAFE auto_accept"
        return "Tier TIC-SAFE manual_review"
    if family == "ticker_year":
        return "Tier TIC diagnostic_only_manual_review"
...
def scenario_status(row, scenario):
    ...
    if scenario == "broad_recall_with_manual_flags":
        return "auto_accept" if tier.startswith(("Tier A auto", "Tier B auto", "Tier C auto", "Tier D auto", "Tier TIC-SAFE auto", "Tier E auto")) else "manual_review"
    return "manual_review"
