# 8-Final-Integration-And-Validation(rev3).py
...
def is_valid_id(value):
    ...
    if text.lower() in {"", "-1", "-1.0", "nan", "none", "null", "<na>"}:
        return False

    numeric_value = pd.to_numeric(text, errors="coerce")
    if not pd.isna(numeric_value) and numeric_value == -1:
        return False
    return True
...
def print_step7_merge_diagnostics(long, step7_lookup):
    ...
    print(f"step5_has_any_identifier count: {int(long['step5_has_any_identifier'].sum())}")
    print(f"step5_unmatched count: {int(step5_unmatched.sum())}")
    ...
    print("llm_decision counts among step5_unmatched rows:")
    print(unmatched["llm_decision"].fillna("").replace("", "(missing)").value_counts(dropna=False).to_string())


def print_final_identifier_diagnostics(long):
    print("Final identifier diagnostics")
    print(f"final_has_any_identifier count: {int(long['final_has_any_identifier'].sum())}")
    print(f"final_unmatched count: {int((~long['final_has_any_identifier']).sum())}")
...
def main():
    ...
    print_final_identifier_diagnostics(long)
