...
OUT_INTERNAL = FINAL_DIR / "final_competitor_mentions_internal_audit_step9_rev1.csv"
OUT_EXTERNAL_IDENTIFIED = FINAL_DIR / "final_competitor_mentions_clean_identified_only_step9_rev1.csv"
OUT_EXTERNAL_ALL_STATUS = FINAL_DIR / "final_competitor_mentions_clean_all_status_step9_rev1.csv"
OUT_EDGES = FINAL_DIR / "final_competitor_edges_clean_identified_only_step9_rev1.csv"
OUT_STATUS_SUMMARY = FINAL_DIR / "final_step9_status_summary_rev1.csv"
OUT_UNIQUE_SUMMARY = FINAL_DIR / "final_step9_unique_name_summary_rev1.csv"
OUT_ERROR_SUMMARY = FINAL_DIR / "final_step9_validation_error_rate_summary_rev1.csv"
OUT_RULE_SUMMARY = FINAL_DIR / "final_step9_rule_extension_summary_rev1.csv"
OUT_UNRESOLVED = FINAL_DIR / "final_step9_internal_unresolved_excluded_from_external_rev1.csv"
OUT_REJECTED_CORRECTIONS = FINAL_DIR / "final_step9_human_rejected_or_corrected_aliases_rev1.csv"
...
def main() -> None:
    ...
    focal_panel = pd.read_csv(IDENTIFIER_YEAR_PANEL, low_memory=False)
    focal_panel = focal_panel[["cik", "gvkey", "fyear"]].copy()
    focal_panel["focal_cik"] = pd.to_numeric(focal_panel["cik"], errors="coerce")
    focal_panel["fyear"] = pd.to_numeric(focal_panel["fyear"], errors="coerce")
    focal_panel["focal_gvkey"] = focal_panel["gvkey"].map(format_identifier)
    focal_panel = (
        focal_panel.dropna(subset=["focal_cik", "fyear"])
        .sort_values(["focal_cik", "fyear", "focal_gvkey"])
        .drop_duplicates(["focal_cik", "fyear"], keep="first")
        [["focal_cik", "fyear", "focal_gvkey"]]
    )
    focal_panel["focal_cik"] = focal_panel["focal_cik"].astype("int64")
    focal_panel["fyear"] = focal_panel["fyear"].astype("int64")
    ...
    enriched = enriched.merge(focal_panel, on=["focal_cik", "fyear"], how="left", validate="many_to_one")
