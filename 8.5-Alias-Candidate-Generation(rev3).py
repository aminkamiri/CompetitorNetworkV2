# 8.5-Alias-Candidate-Generation(rev3).py
...
LEGAL_FORM_NORMALIZATION_TERMS = {
    ...
    "SPA", "GROUP", "GRP", "HOLDING", "HLDG", "COMPANY",
}

PARENT_LEGAL_ENTITY_TERMS = LEGAL_FORM_NORMALIZATION_TERMS | {
    "HLDGS", "HOLDINGS", "COMPANY", "LIMITED", "LLC", "GMBH", "KGAA", "BV",
    "AB", "AS", "SAS", "SARL", "SRL", "PTY", "PTE", "BHD",
}
...
LEGAL_FORM_SUFFIXES = {
    ...
    "SPA", "PTE", "PVT", "SL", "SAU", "SPRL", "KS", "AB", "AS", "APS",
    "ASA", "BVBA", "KK", "LTDA", "RT", "ZRT", "KFT", "KG", "PTY", "BHD",
    "SDN", "PJSC", "JSC", "OAO", "ZAO", "AO", "SE", "COMPANY",
}
...
def parent_prefix_is_strong(row):
    ...
    candidate_legal_stripped = strip_legal_form_suffixes(standardize_name_for_mapping(row.get("candidate_bridge_name", "")))
    alias_legal_stripped = strip_legal_form_suffixes(standardize_name_for_mapping(row.get("suggested_alias_name", "")))
    return candidate_legal_stripped != "" and candidate_legal_stripped == alias_legal_stripped
...
def print_target_alias_candidate_checks(lookups):
    ...
    for alias in target_aliases:
        ...
        print(f"{alias}: {'FOUND' if candidates else 'NO_CANDIDATE'} ({len(candidates)})")
