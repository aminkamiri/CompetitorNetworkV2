        ...
        "classifications": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "suggested_alias_name": {"type": ["string", "null"]},
                    "suggested_alias_scope": {"type": "string", "enum": sorted(VALID_ALIAS_SCOPES)},
                    "suggested_alias_reason": {"type": "string"},
                },
                "required": [
                    "record_id",
                    "competitor_name",
...
def normalize_result(item, input_row=None, reason_override=None):
    ...
    alias_scope = str(item.get("suggested_alias_scope", "unclear")).strip()
    if alias_scope not in VALID_ALIAS_SCOPES:
        alias_scope = "unclear"

    suggested_alias_name = item.get("suggested_alias_name", "")
    if suggested_alias_name is None:
        suggested_alias_name = ""
    suggested_alias_name = str(suggested_alias_name).strip()

    confidence = normalize_confidence(item.get("llm_confidence", 0))
    needs_manual_check = (confidence < 85) or (decision == "manual_review")
    ...
    return {
        **input_row,
        ...
        "suggested_alias_name": suggested_alias_name,
        "suggested_alias_scope": alias_scope,
        "suggested_alias_reason": str(item.get("suggested_alias_reason", "")).strip(),
    ...
    return normalize_result(
        {
            ...
            "suggested_alias_name": "",
            "suggested_alias_scope": "unclear",
            "suggested_alias_reason": "Missing from model output",
        },
        input_row=row_dict,
    )
...
def build_batch_prompt(batch):
    ...
    return {
        ...
        "hard_rules": [
            ...
            "Do not assign CIK, GVKEY, PERMNO, ticker, or any identifier.",
            "The alias is only a search key for deterministic candidate generation in Step 8.",
            "If the name is a subsidiary/business unit, do not automatically collapse it to parent unless the parent is explicitly present in the extracted name.",
            "If the name is ambiguous across multiple firms, use manual_review and suggested_alias_scope unclear or multiple_entities.",
        ...
        "allowed_alias_scopes": sorted(VALID_ALIAS_SCOPES),
        "alias_guidance": {
            "suggested_alias_name": "A cleaned organization-like search key for DISCERN/CCM/Compustat Global. No identifiers. Leave blank for generic/drop cases unless useful.",
            "suggested_alias_scope": "One of the allowed alias scopes.",
            "suggested_alias_reason": "Short explanation of why the alias is useful or why it is blank."
        },
        "examples": [
            {"competitor_name": "GlaxoSmithKline plc", "suggested_alias_name": "GlaxoSmithKline", "suggested_alias_scope": "parent_company"},
            {"competitor_name": "Eli Lilly & Co.", "suggested_alias_name": "Eli Lilly", "suggested_alias_scope": "parent_company"},
            {"competitor_name": "Wal-Mart Stores, Inc.", "suggested_alias_name": "Wal-Mart", "suggested_alias_scope": "parent_company"},
            {"competitor_name": "Novartis International AG", "suggested_alias_name": "Novartis", "suggested_alias_scope": "parent_company"},
            {"competitor_name": "Roche Diagnostics", "suggested_alias_name": "Roche Diagnostics", "suggested_alias_scope": "subsidiary_or_business_unit"},
            {"competitor_name": "Bing", "suggested_alias_name": "", "suggested_alias_scope": "brand_or_product"}
        ],
        "decision_rules": {
        ...
        "confidence_rule": "Return llm_confidence as an integer from 0 to 100.",
        "records": records,
    }
...
def load_existing_output(overwrite):
    ...
    existing = existing.drop_duplicates(subset=["record_id"], keep="last")
    return ensure_needs_manual_check(existing)


def ensure_needs_manual_check(df):
    if df.empty:
        return df
    ...
    df["llm_confidence"] = df["llm_confidence"].apply(normalize_confidence)

    if "llm_decision" not in df.columns:
        df["llm_decision"] = "manual_review"

    if "suggested_alias_name" not in df.columns:
        df["suggested_alias_name"] = ""

    if "suggested_alias_scope" not in df.columns:
        df["suggested_alias_scope"] = "unclear"
    else:
        df["suggested_alias_scope"] = df["suggested_alias_scope"].fillna("").astype(str)
        df.loc[
            ~df["suggested_alias_scope"].isin(VALID_ALIAS_SCOPES),
            "suggested_alias_scope"
        ] = "unclear"

    if "suggested_alias_reason" not in df.columns:
        df["suggested_alias_reason"] = ""

    df["needs_manual_check"] = (
        (df["llm_confidence"] < 85) |
        (df["llm_decision"].astype(str) == "manual_review")
    )
...
def save_outputs(result, issues):
    ...
    result = ensure_needs_manual_check(result)
    ...
    safe_to_csv(
        result[result["needs_manual_check"]].copy(),
        STEP7_DIR / "step7_needs_manual_check.csv",
        index=False,
    )

    summary = (
        result.groupby(["llm_classification", "llm_decision", "suggested_alias_scope"], dropna=False)
        .agg(records=("record_id", "count"), total_frequency=("frequency", "sum"))
        .reset_index()
        if not result.empty
        else pd.DataFrame(columns=["llm_classification", "llm_decision", "suggested_alias_scope", "records", "total_frequency"])
    )
    ...
    sample_specs = [
        ("llm_decision", "accept_for_manual_alias_bridge", "accepted_for_manual_alias_bridge_sample.csv"),
        ("llm_decision", "drop", "dropped_cases_sample.csv"),
        ("llm_decision", "manual_review", "manual_review_cases_sample.csv"),
        ("needs_manual_check", True, "needs_manual_check_sample.csv"),
    ]
    for column, value, filename in sample_specs:
        sample = result[result[column] == value].copy()
        if sample.empty:
...
def main():
    ...
    print("Total Step 7 input rows:", input_rows)
    print("Rows selected after max_records:", len(df))
    print("Already processed rows:", len(already_processed))
    print("Remaining rows to API:", len(to_process))
    ...
    print("Final output rows:", len(result))
